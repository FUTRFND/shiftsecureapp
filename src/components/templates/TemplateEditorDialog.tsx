...
export function TemplateEditorDialog({
  ...
  const addSection = () =>
    setForm((f) => ({
      ...
      sections: [
        ...f.sections,
        { id: newSectionId(), title: "", placeholder: "", required: false },
      ],
    }));
  ...
  const applyPreset = (label: string) => {
    ...
    if (preset)
      setForm({
        ...preset.template,
        sections: preset.template.sections.map((s) => ({ ...s, id: newSectionId() })),
      });
  };
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="space-y-2 sm:col-span-2">
                ...
                <Input
                  ...
                  placeholder="e.g. ED Resus Handoff"
                />
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {form.sections.length} item{form.sections.length === 1 ? "" : "s"}
                </span>
              </div>
              {errors["sections"] && (
                <p className="text-sm text-destructive mb-2">{errors["sections"]}</p>
              )}
              <div className="space-y-3">
                {form.sections.map((section, idx) => (
                  <div
                    key={section.id}
                    className="rounded-lg border border-border/60 bg-card/60 p-3 space-y-3"
                  >
                    <div className="flex items-start gap-2">
                      <div className="flex flex-col text-muted-foreground pt-2">
                        <button
                          type="button"
                          onClick={() => move(idx, -1)}
                          className="hover:text-foreground text-xs leading-none"
                          aria-label="Move up"
                        >
                          ▲
                        </button>
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            updateSection(section.id, { placeholder: e.target.value })
                          }
